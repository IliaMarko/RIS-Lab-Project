...
from myrov_control.srv import CmdThruster,CmdThrusterResponse      ################
...
class ThrusterAllocatorNode(ThrusterManager):
    ...
    def __init__(self):
        ...
        #self.input_sub = rospy.Subscriber('thruster_manager/input',Wrench, self.input_callback)
        ...
        self.service = rospy.Service('cmd_thruster', CmdThruster, self.input_callback)
        print("Ready to convert wrench data.")
    ...
    def input_callback(self,req):
        ...
        force_vec=req.force
        torque_vec=req.torque
        print("Recive message correct")
        force = numpy.array((force_vec.x,force_vec.y,force_vec.z))
        torque = numpy.array((torque_vec.x, torque_vec.y,torque_vec.z))
...
if __name__ == '__main__':
    rospy.init_node('thruster_cmd_pub_server')
    try:
